...
```{r}
prediction_tue_rf <- predict(rf_classe, tune_data)
...
```{r}
prediction_test <- predict(rf_classe, test_data)
cM_test_rf <- confusionMatrix(prediction_rf, test_data$classe)
```
Prediction on test partition dataset (15%).
